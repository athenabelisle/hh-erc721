...
contract RandomIpfsNft is ERC721URIStorage, VRFConsumerBaseV2, Ownable {
    ...
    constructor(
        ...
        _initializeContract(artTokenUris);
    }
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        ...
        Art artType = getArtFromModdedRng(moddedRng);
        _safeMint(nftOwner, newItemId);
        _setTokenURI(newItemId, s_artTokenUris[uint256(artType)]);
        emit NftMinted(artType, nftOwner);
    }
    ...
    function _initializeContract(string[3] memory artTokenUris) private {
        if (s_initialized) {
